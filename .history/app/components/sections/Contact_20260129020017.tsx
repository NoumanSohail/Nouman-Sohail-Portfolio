...
const Contact = () => {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    await sendEmail(form.name, form.email, form.phone, form.message);
    setLoading(false);
  };
            ...
            >
              ...
              >
                <GlassCard>
                  ...
                  <Button
                    title={`${loading ? "Submitting..." : "Submit Message"}`}
                    onClick={handleSubmit}
                    className="bg-white/5 border border-white/20 p-3 rounded-full md:text-lg backdrop-blur-xs"
                    disabled={loading}
                  />
                </GlassCard>
              </motion.div>
