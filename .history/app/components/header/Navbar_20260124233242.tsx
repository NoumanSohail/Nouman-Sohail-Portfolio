  ...
  return (
    ...
    >
      <div className="w-full mx-auto max-w-384 flex items-center justify-between px-5 md:px-10 lg:px-15 text-white tracking-widest">
        <h1 className="font-bold text-2xl border border-gray-200">Portfolio</h1>

        {/* large displays like laptops */}
        <nav className="hidden lg:flex">
          {navLinks.map((links) => (
            <div key={links.name} className="flex ml-10 font-semibold">
              <Link href={links.id}>{links.name}</Link>
            </div>
          ))}
        </nav>
